  ...
  try {
    ...
    if (result.statusCode === StatusCode.SUCCESS) {
      ...
      const token = jwt.sign(
        { admin_name: admin_name, user_id: (result.data as Admin).admin_id },
        secret,
        { expiresIn: "1h" }
      );
      const response = Response.json(
        {
          ...result,
          data: { user_id: (result.data as Admin).admin_id, admin_name },
        },
        { status: 200, headers: {} }
      );
      ...
      return response;
    } else if (result.statusCode === StatusCode.DB_ERROR) {
      // see else can be used
      const response = Response.json(result, { status: 500 });
      return response;
    } else if (result.statusCode === StatusCode.NO_USERS_FOUND) {
      // see else can be used
      const response = Response.json(result, { status: 500 });
      return response;
    } else {
