    ...
    try {
        ...
        if (!admin_name || !password) {
            return Response.json(
                { message: "admin_name and password are required", data: undefined, statusCode: StatusCode.INVALID_INPUT } as SignUpResponseBody,
                { status: 400 }
            );
        }
        ...
        if(result.statusCode ===  StatusCode.NEW_USER_CREATED) {
            ...
            const token = jwt.sign({ admin_name: admin_name, user_id: result.data }, secret, { expiresIn: '1h' });
